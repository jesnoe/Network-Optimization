...
t_test_summary <- read.csv("Cocaine Network Optimization/t_test_summary (2009-2014).csv") %>% as_tibble
}
...
for (alpha_i in seq(1, 0, by=-0.1)) {
  for (epsilon_si in seq(1, 0, by=-.2)) {
    ...
    opt_sol %>% write.csv(paste0("Cocaine Network Optimization/Results/Equal mean test (Sig lev=",
                                 alpha_t_test,
                                 ")/optimal solution equal mean test epsilton_s ",
    ...
    ggsave(paste0("Cocaine Network Optimization/Results/Equal mean test (Sig lev=",
                  alpha_t_test,
                  ")/optimal networks equal mean test epsilton_s ",
                  period[1], "-", period[length(period)],
                  " (alpha=",
                  alpha_i,
                  ", epsilton_s=",
...
alpha_t_test <- 0.5
{
  ...
  relaxed_pairs_index <- relaxed_pairs_index %>% filter(source_index < destination_index)
  zero_d_vars_index <- unique(less_than_price) %>% sort
  zero_d_vars_index <- zero_d_vars_index[-2]
  n_zero_d_vars <- length(zero_d_vars_index)
}

relaxed_pairs_index_AZ_CA <- rbind(relaxed_pairs_index, c(2,4))
for (alpha_i in seq(1, 0, by=-0.1)) {
  for (epsilon_si in seq(1, 0, by=-.2)) {
    model_epsilon_s <- list()
    model_epsilon_s$obj        <- c(rep(1-alpha_i, n_d_vars - n_zero_d_vars - 2*N), rep(0, N), rep(alpha_i, N))
    ...
    model_epsilon_s$sos <- vector(mode="list", length=nrow(relaxed_pairs_index_AZ_CA))
    for (j in 1:nrow(relaxed_pairs_index_AZ_CA)) {
      model_epsilon_s$sos[[j]]$type <- 1
      model_epsilon_s$sos[[j]]$index <- which(d_vars[-zero_d_vars_index] %in% c(paste0("w",
                                                                                           relaxed_pairs_index_AZ_CA$source_index[j],
                                                                                           ",",
                                                                                           relaxed_pairs_index_AZ_CA$destination_index[j],
                                                                                           "."),
                                                                                    paste0("w",
                                                                                           relaxed_pairs_index_AZ_CA$destination_index[j],
                                                                                           ",",
    ...
    opt_sol %>% write.csv(paste0("Cocaine Network Optimization/Results/Equal mean test AZ=CA (Sig lev=",
                                 alpha_t_test,
                                 ")/optimal solution equal mean test epsilton_s ",
                                 period[1], "-", period[length(period)],
                                 " (alpha=",
    ...
    overdose_map_year %>% ggplot() +
      ...
      geom_text(data=opt_sol[result_source_index,],
                ...
                color="red",
                nudge_x = c(0.1, 0), nudge_y = c(-0.5, 0)) -> optimal_network_map
    ggsave(paste0("Cocaine Network Optimization/Results/Equal mean test AZ=CA (Sig lev=",
                  alpha_t_test,
                  ")/optimal networks equal mean test epsilton_s ",
                  period[1], "-", period[length(period)],
                  " (alpha=",
