...
library(gridExtra)
library(lubridate)
#### need to address triangle
#### need to add SOS for w_ij != w_ji
{
...
overdose_period <- overdose %>% 
  ...
  group_by(state) %>% 
  summarise(avg_deaths_T40.5=mean(deaths_T40.5, na.rm=T),
            med_deaths_T40.5=median(deaths_T40.5, na.rm=T),
            avg_deaths_T40.5_only=mean(deaths_T40.5_only, na.rm=T),
            med_deaths_T40.5_only=median(deaths_T40.5_only, na.rm=T))
...
states_data$med_deaths_T40.5_only <- ifelse(is.na(states_data$med_deaths_T40.5_only), 0, states_data$med_deaths_T40.5_only)  
...

{
  ...
  O <- states_data$med_deaths_T40.5
  O_only <- states_data$med_deaths_T40.5_only
  ...
  relaxed_n_zero_d_vars <- length(relaxed_zero_d_vars_index)
...
# results with varying alpha (no relaxation)
...
for (alpha in seq(1, 0, by=-0.1)) {
  ...
  model_alpha$A          <- rbind(A1[,-zero_d_vars_index], A2[,-zero_d_vars_index], A3[,-zero_d_vars_index], A_source_limit[,-zero_d_vars_index])
  ...
  # model_alpha$rhs        <- c(b1, b2_ratio, b3, b_source_limit)
  model_alpha$rhs        <- c(b1_only, b2_ratio, b3, b_source_limit)
...
alpha_results # all infeasible with either T40.5 or T40.5 only

# results with varying alpha (price & purity relaxation)
alpha_results <- data.frame()
for (alpha in seq(1, 0, by=-0.1)) {
  model_alpha <- list()
  model_alpha$obj        <- c(rep(1-alpha, n_d_vars - relaxed_n_zero_d_vars - 2*N), rep(0, N), rep(alpha, N))
  model_alpha$modelsense <- "min"
  model_alpha$A          <- rbind(A1[,-relaxed_zero_d_vars_index], A2[,-relaxed_zero_d_vars_index],
                                  A3[,-relaxed_zero_d_vars_index], A_source_limit[,-relaxed_zero_d_vars_index])
  model_alpha$sense      <- rep(">", nrow(model_alpha$A))
  model_alpha$sense[1:N] <- "="
  model_alpha$sense[nrow(model_alpha$A)] <- "="
  model_alpha$vtype      <- c(rep("C", length(model_alpha$obj) - N), rep("B", N)) # Continuous and binary
...
# epsilon_seizure test (no relaxation)
alpha <- 1
...
for (epsilon_s in seq(0, 1, by=0.02)) {
  ...
  # model_epsilon_s$rhs        <- c(b1_only, epsilon_s*b2_ratio, b3, b_source_limit)
...
epsilon_s_results # max(epsilon_s)=0.1 with T40.5 and max(epsilon_s)=0.04 with T40.5 only 

# epsilon_seizure test (price & purity relaxation)
alpha <- 1
epsilon_s_results <- data.frame()
for (epsilon_s in seq(0, 1, by=0.02)) {
  model_epsilon_s <- list()
  model_epsilon_s$obj        <- c(rep(1-alpha, n_d_vars - relaxed_n_zero_d_vars - 2*N), rep(0, N), rep(alpha, N))
  model_epsilon_s$modelsense <- "min"
  model_epsilon_s$A          <- rbind(A1[,-relaxed_zero_d_vars_index], A2[,-relaxed_zero_d_vars_index],
                                      A3[,-relaxed_zero_d_vars_index], A_source_limit[,-relaxed_zero_d_vars_index])
  model_epsilon_s$sense      <- rep(">", nrow(model_epsilon_s$A))
  model_epsilon_s$sense[1:N] <- "="
  model_epsilon_s$sense[nrow(model_epsilon_s$A)] <- "="
...
names(epsilon_s_results) <- c("epsilon_s", "feasibility", "n_opt_sols")
epsilon_s_results # max(epsilon_s)=0.16 with T40.5 and max(epsilon_s)=0.08 with T40.5 only (0.1 sig. lev.for equality test)
...
for (alpha_i in seq(1, 0, by=-.1)) {
  for (epsilon_si in seq(.4, .1, by=-.1)) {
    ...
    overdose_map_year <- left_join(states %>%
                                     rename(state=state_name) %>% 
                                     filter(!(state %in% c("Alaska", "Hawaii"))),
                                   states_data %>% select(state, med_deaths_T40.5),
                                   by="state")
    overdose_map_year %>% ggplot() +
      geom_polygon(aes(x=long,
                       y=lat,
                       group=group,
                       fill=med_deaths_T40.5),
                   color="black") +
