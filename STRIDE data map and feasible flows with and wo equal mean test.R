...
for (period in periods) {
  ...
  allowed_flows <- data.frame(deicision_vars=d_var_price_restricted)
  ...
  allowed_flows <- data.frame(deicision_vars=d_var_purity_restricted)
  allowed_flows <- cbind(allowed_flows, find_state_index(allowed_flows$deicision_vars) %>% t) %>% 
    rename(i="1", j="2")
...
length(less_than_price_no_test)     # 109
length(greater_than_purity_no_test) # 109
intersect(less_than_price_no_test, greater_than_purity_no_test) %>% length # 65
